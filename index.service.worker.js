 ...
 * (/mega-soccer/). Clients that still have that worker will fetch this file,
 * uninstall themselves, drop only Arcade Soccer caches, and reload once.
 */
...
self.addEventListener("activate", function (event) {
	event.waitUntil((async function () {
		...
		await Promise.all(keys.filter(function (k) {
			return k.indexOf("Arcade Soccer-sw-cache-") === 0
